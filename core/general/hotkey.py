...
class hotkey:
    ...
    def setup(self, command, name=None, sourceType='python', annotation=''):
        ...
        # Default using first part of command if name is not explicitly given
        if not name:
            # remove non alphanumeric characters from command for name
            name = re.sub(r'\W+', '', command.split()[0])
    ...
    def setupHotkey(self, name, sourceType='mel'):
        ...
        self.name = name
        # Make names consistant with Maya's convention
        self.commandName = name + 'NameCommand'
        self.sourceType = sourceType

        if self.release:
            cmds.hotkey(k=self.key, alt=self.alt,
                        ctl=self.ctl, cmd=self.cmd, rn=self.commandName)
        else:
            cmds.hotkey(k=self.key, alt=self.alt,
                        ctl=self.ctl, cmd=self.cmd, n=self.commandName)

        cmds.nameCommand(
            self.commandName, ann=self.commandName, c=self.name, stp=self.sourceType)
    ...
    def setupCommand(self, command, name, sourceType, annotation):
        ...
        cmds.runTimeCommand(self.name, ann=self.annotation,
                            cat='User', c=self.command, cl=self.sourceType)
